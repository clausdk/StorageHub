...
| Security | Versioned vault, Windows DPAPI current-user protector, secret leases, trust-store contracts, SQLite repository, atomic trust rollover, and a shared key/certificate store with agent-derived summaries, reference-counted bindings, and metadata-only IPC |
| Persistence | Cross-process-serialized ordered migrations; strict transactional archival of the recognized legacy-preview schema collision; WAL/foreign-key/full-sync configuration; integrity checks; single-writer boundary; profile/trust, scheduler, transfer, sync, execution, reliable-outbox, and key-store repositories through schema v14 |
...
| Desktop | Workspaces of one to four panes with navigation, history, and bounded paging; file copy/move enqueue; create, rename, batch rename, and delete; durable queue, sync, and schedule surfaces; read-only versions/metadata/tags inspector; Explorer drag and drop with determinate progress |
| Connection Manager | Grouped, sorted profile tree with folders, favorites, tags, and search; protocol-aware editing with provider-enforceable defaults; key store management and per-slot key selection; verified certificate and host-key enrollment, rejection, and rollover; manual, ask, or automatic SSH host-key discovery; explicit vault enrollment; revision-bound health presentation |
| Settings | Transfers and sync, editing, appearance, workspace, rebindable shortcuts, catalog-driven per-provider connection defaults and trust policy, and updates. Light, Dark, and System themes apply to every window including native scrollbars, list headers, spin buttons, and edit borders. Settings, connections, and sync tasks export and import with optional password protection |
...
1. Extend the implemented recursive saved-connection folder copy to deliberately
   ...
   create-only.
2. Add a bounded temporary-session contract before restoring Quick Connect.
   Pane comparison and global queue controls remain hidden until their real
   command paths and failure handling exist.
