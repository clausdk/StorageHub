...
public sealed class DatabaseFoundationTests : IDisposable
{
    ...
    public async Task Initialize_creates_versioned_schema_with_durable_settings()
    {
        ...
        Assert.Equal(LocalTransferEndpointsSchemaMigration.SchemaVersion, result.SchemaVersion);
        ...
        Assert.Equal(LocalTransferEndpointsSchemaMigration.SchemaVersion, await ScalarInt64Async(connection, "PRAGMA user_version;"));
    ...
    public async Task Initialize_is_idempotent_and_records_each_migration_once()
    {
        ...
        Assert.Equal(12L, await ScalarInt64Async(connection, "SELECT COUNT(*) FROM schema_migrations;"));
    }
