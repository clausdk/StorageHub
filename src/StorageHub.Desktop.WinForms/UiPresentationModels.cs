...
public sealed record SettingsCategoryDescriptor(string Name, string Summary, IReadOnlyList<SettingDescriptor> Settings)
{
    public override string ToString() => Name;
}
...
public static class SettingsPresentationCatalog
{
    private static readonly ReadOnlyCollection<SettingsCategoryDescriptor> Categories = Array.AsReadOnly(
    new SettingsCategoryDescriptor[]
    {
        Category("General", "Startup, session restore, and default workspace behavior",
            Setting("restoreSession", "Restore previous workspace", "Reopen tabs and non-secret locations after startup.", ConnectionFieldKind.Toggle, "true"),
            Setting("confirmExit", "Confirm while jobs are active", "Warn before closing the desktop while the agent still has work.", ConnectionFieldKind.Toggle, "true")),
        Category("Interface", "Theme, density, accessibility, and file-list presentation",
            Setting("theme", "Theme", "Follow Windows by default.", ConnectionFieldKind.Choice, "System", ["System", "Light", "Dark"]),
            Setting("density", "Layout density", "Adjust row and command spacing.", ConnectionFieldKind.Choice, "Comfortable", ["Compact", "Comfortable", "Spacious"]),
            Setting("reducedMotion", "Reduce motion", "Avoid nonessential progress animations.", ConnectionFieldKind.Toggle, "false")),
        Category("Connections", "Connection behavior shared by all providers",
            Setting("connectTimeout", "Connect timeout (seconds)", "Maximum time allowed for a connection handshake.", ConnectionFieldKind.Number, "30"),
            Setting("idleTimeout", "Idle disconnect (minutes)", "Disconnect inactive sessions without deleting queued work.", ConnectionFieldKind.Number, "15")),
        Category("FTP / FTPS", "Passive mode, TLS, and compatibility defaults",
            Setting("ftpPassive", "Prefer passive mode", "Recommended for clients behind NAT or firewalls.", ConnectionFieldKind.Toggle, "true"),
            Setting("ftpTls", "Default transport", "New profiles should use explicit TLS.", ConnectionFieldKind.Choice, "Explicit TLS", ["Explicit TLS", "Implicit TLS", "Plain FTP with warning"])),
        Category("SFTP / SSH", "Host-key verification and SSH session defaults",
            Setting("sshStrict", "Require verified host key", "Never accept a first-seen host key silently.", ConnectionFieldKind.Toggle, "true"),
            Setting("sshKeepAlive", "Keep-alive (seconds)", "Interval for idle SSH sessions.", ConnectionFieldKind.Number, "30")),
        Category("S3 / Object Storage", "Multipart, addressing, and object-list defaults",
            Setting("s3Multipart", "Multipart threshold (MiB)", "Use resumable multipart upload for larger objects.", ConnectionFieldKind.Number, "64"),
            Setting("s3Addressing", "Addressing style", "Virtual-hosted style is preferred when supported.", ConnectionFieldKind.Choice, "Virtual-hosted", ["Virtual-hosted", "Path-style"])),
        Category("Transfers", "Concurrency, retries, bandwidth, and integrity",
            Setting("globalConcurrency", "Global concurrency", "Maximum transfers across all connections.", ConnectionFieldKind.Number, "4"),
            Setting("perConnectionConcurrency", "Per-connection concurrency", "Maximum transfers sharing one endpoint.", ConnectionFieldKind.Number, "2"),
            Setting("verification", "Verification", "Balanced verifies metadata and available provider checksums.", ConnectionFieldKind.Choice, "Balanced", ["Fast", "Balanced", "Full checksum"]),
            Setting("temporaryTargets", "Use safe temporary targets", "Commit downloads only after successful completion.", ConnectionFieldKind.Toggle, "true")),
        Category("Sync", "Comparison, conflict, and deletion safety defaults",
            Setting("deletePropagation", "Delete propagation", "Disabled for every new profile until explicitly reviewed.", ConnectionFieldKind.Toggle, "false"),
            Setting("massDeleteCount", "Mass-delete item limit", "Pause a plan that exceeds this number.", ConnectionFieldKind.Number, "100"),
            Setting("massDeletePercent", "Mass-delete percentage limit", "Pause a plan that exceeds this baseline percentage.", ConnectionFieldKind.Number, "10")),
        Category("Scheduling", "Background schedule, catch-up, and power behavior",
            Setting("missedRuns", "Missed-run behavior", "Coalesce missed runs into one safe catch-up.", ConnectionFieldKind.Choice, "Run once", ["Run once", "Skip", "Ask when desktop opens"]),
            Setting("wakeComputer", "Wake computer", "Allow scheduled jobs to wake Windows when supported.", ConnectionFieldKind.Toggle, "false")),
        Category("Security", "Vault, trust records, and sensitive diagnostics",
            Setting("clipboardTimeout", "Clear copied secrets (seconds)", "Remove secrets copied through explicit reveal actions.", ConnectionFieldKind.Number, "30"),
            Setting("diagnosticRedaction", "Redact diagnostics", "Exclude vaults, credentials, private keys, and provider payloads.", ConnectionFieldKind.Toggle, "true")),
        Category("Notifications", "Desktop notifications for completed and failed work",
            Setting("failureNotifications", "Notify on failure", "Show a Windows notification when a job needs attention.", ConnectionFieldKind.Toggle, "true")),
        Category("Logging", "Retention and diagnostic detail",
            Setting("logRetention", "Retention (days)", "Remove old application logs, never transfer data.", ConnectionFieldKind.Number, "14")),
        Category("Cache", "Metadata cache sizing and cleanup",
            Setting("cacheLimit", "Metadata cache (MiB)", "Maximum local metadata cache size.", ConnectionFieldKind.Number, "512")),
        Category("Providers", "CL.Storage provider availability and health",
            Setting("providerHealth", "Health checks", "Check enabled providers when the agent starts.", ConnectionFieldKind.Toggle, "true")),
        Category("Advanced", "Recovery and developer-focused options",
            Setting("recoveryMode", "Offer recovery mode", "Start with diagnostics available if a required subsystem fails.", ConnectionFieldKind.Toggle, "true"))
    });

    public static IReadOnlyList<SettingsCategoryDescriptor> All => Categories;

    private static SettingsCategoryDescriptor Category(string name, string summary, params SettingDescriptor[] settings) =>
        new(name, summary, new ReadOnlyCollection<SettingDescriptor>(settings));

    private static SettingDescriptor Setting(
        string key,
        string label,
        string description,
        ConnectionFieldKind kind,
        string defaultValue,
        IReadOnlyList<string>? choices = null) =>
        new(key, label, description, kind, defaultValue, choices);
}
